...
class Install {
    ...
    protected function StepDb() {
        ...
        if($this->ValidateDBConnection($aParams)) {
            ...
            if($aParams['engine']=='InnoDB') {
                ...
                if($aRes = @mysqli_query($this->oDb,'SHOW ENGINES')) {
    ...
    protected function StepAdmin() {
        ...
        if(!$bUpdated) {
            $this->aMessages[] = array('type'=>'error','text'=>$this->Lang('error_db_saved').'<br />'.mysqli_error($this->oDb));
            $this->Layout('steps/admin.tpl');
            return false;   
        }
        /**
            ...
            if($aParams['install_lang_current'] && strlen($aParams['install_lang_current'])>1){
                if($this->SaveConfig('lang.current',$aParams['install_lang_current'],$sLocalConfigFile)) {
                    ...
                    if($aParams['install_lang_current']=='russian'){ 
                        ...
                        if ($this->ValidateDBConnection($aDbParams) and $this->SelectDatabase($aDbParams['name'])) {
                            $this->UpdateUserBlog("Блог им. ".$this->GetSessionVar('install_admin_login'),$aDbParams['prefix']);
                        }
                    }
                }
            } else {
    ...
    protected function SelectDatabase($sName,$bCreate=false) {
        if (@mysqli_select_db($this->oDb, $sName)) {
            return true;
        }

        if ($bCreate) { 
            @mysqli_query($this->oDb, "CREATE DATABASE $sName");
            return @mysqli_select_db($this->oDb, $sName);
        } 
        return false;
    }
    /**
    ...
    protected function CreateTables($sFilePath,$aParams) {
         ...
         */ 
        $aDbTables = array();
        $aResult = @mysqli_query($this->oDb,"SHOW TABLES");
        if(!$aResult){  
        ...
        foreach($aQuery as $sQuery){
            ...
            if($sQuery!='' and !$this->IsUseDbTable($sQuery,$aDbTables)) {
                $bResult=mysqli_query($this->oDb,$sQuery);
                if(!$bResult) $aErrors[] = mysqli_error($this->oDb);
            }
        }
    ...
    protected function ConvertDatabase($sFilePath,$aParams) {   
        ...
        $sQuery = "
            ...
            ";
        if(!mysqli_query($this->oDb,$sQuery)) $aErrors[] = mysqli_error($this->oDb);
        /**
        ...
        do {
            ...
            if(!$aResults = mysqli_query($this->oDb,$sQuery)){
                $aErrors[] = mysqli_error($this->oDb);
                break;
            }
            if (mysqli_num_rows($aResults)) {
                while($aRow = mysqli_fetch_assoc($aResults)) {
                    ...
                    if(!$aResults2 = mysqli_query($this->oDb,$sQuery2)){
                        $aErrors[] = mysqli_error($this->oDb);
                        continue;
                    }
                    ...
                    if(!mysqli_query($this->oDb,$sQuery3)) {
                        $aErrors[] = mysqli_error($this->oDb);
                        continue;
                    }
                }
        ...
        do {
            ...
            if (mysqli_num_rows($aResults)) {
                while($aRow = mysqli_fetch_assoc($aResults)) {
                    ...
                    $sCountry=mysqli_real_escape_string($this->oDb,$aRow['user_profile_country']);
                    $sCity=mysqli_real_escape_string($this->oDb,(string)$aRow['user_profile_city']);
                    /**
                    ...
                    if($aRow2 = mysqli_fetch_assoc($aResults2)) {
                        $iCountryId=$aRow2['id'];
                        $sCountryName=mysqli_real_escape_string($this->oDb,$aRow2['name_ru']);
                    } else {
                        $sQuery2="UPDATE {$sTableUser} SET user_profile_country=null, user_profile_region=null, user_profile_city=null WHERE user_id={$iUserId} ";
                        if(!$aResults2 = mysqli_query($this->oDb,$sQuery2)){
                            $aErrors[] = mysqli_error($this->oDb);
                    ...
                    if ($sCity) {
                        ...
                        if($aRow2 = mysqli_fetch_assoc($aResults2)) {
                            $iCityId=$aRow2['id'];
                            $sCityName=mysqli_real_escape_string($this->oDb,$aRow2['name_ru']);
                            $iRegionId=$aRow2['region_id'];
                            /**
                             * Получаем название региона
                             */
                            $sQuery3="SELECT name_ru FROM {$sTableGeoRegion} WHERE id='{$iRegionId}' LIMIT 0,1";
                            if(!$aResults3 = mysqli_query($this->oDb,$sQuery3)){
                                $aErrors[] = mysqli_error($this->oDb);
                                continue;
                            }
                            if($aRow3 = mysqli_fetch_assoc($aResults3)) {
                                $sRegionName=mysqli_real_escape_string($this->oDb,$aRow3['name_ru']);
                            } else {
                                continue;
                            }
                        }
            ...
            do {
                ...
                if (mysqli_num_rows($aResults)) {
                    while($aRow = mysqli_fetch_assoc($aResults)) {
                        ...
                        if ($sIcq) {
                            $sIcq=mysqli_real_escape_string($this->oDb,$sIcq);
                            /**
                             * Проверяем отсутствие связи
                             */
                        ...
                        if ($sWww) {
                            $sWww=mysqli_real_escape_string($this->oDb,$sWww);
                            /**
                             * Проверяем отсутствие связи
                             */
        ...
        if(!mysqli_query($this->oDb,$sQuery)){
            $aErrors[] = mysqli_error($this->oDb);
        }
        ...
        do {
            ...
            if (mysqli_num_rows($aResults)) {
                while($aRow = mysqli_fetch_assoc($aResults)) {
                    ...
                    $sText=mysqli_real_escape_string($this->oDb,$aRow['topic_tag_text']);
                    /**
                     * Проверяем наличие
                     */
        ...
        do {
            ...
            if (mysqli_num_rows($aResults)) {
                while($aRow = mysqli_fetch_assoc($aResults)) {
                    $sAbout=mysqli_real_escape_string($this->oDb,htmlspecialchars(strip_tags($aRow['user_profile_about'])));
                    $iUserId=$aRow['user_id'];
                    /**
                     * Обновляем информацию о пользователе
                     */
    ...
    public function addEnumTypeDatabase($sTableName,$sFieldName,$sType) {
        ...
        if ($res=mysqli_query($this->oDb,$sQuery)) {
    ...
    protected function UpdateUserBlog($sBlogName,$sPrefix="prefix_") {
        $sQuery = "
            UPDATE `{$sPrefix}blog`
            SET 
                `blog_title`    = '".mysqli_real_escape_string($this->oDb,$sBlogName)."'
            WHERE `blog_id` = 1";
